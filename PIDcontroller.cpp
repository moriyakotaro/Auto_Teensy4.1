...
void PID::setTSample(float t){
 ...
 float PID::compute(){
    ...
    //アンチワインドアップ
    if (!(prevControllerOutput_ >= 1 && Error_ > 0) && !(prevControllerOutput_ <= -1/*0->-1に変更*/ && Error_ < 0)) {
    ...
    scaledBias *= (Error_ >= scaledBias);
    scaledBias *= (Error_ > 0) ? 1:-1;
    ...
    //PIDの出力を実際の値に変換して返す
    return (((controllerOutput_ + 0.5/*0.5を追加，負の出力に対応*/) * outSpan_) + outMin_);
 ...
 float PID::scaledParcent(float value) {
    ...
    if(value > 1.0) {
        return 1.0;
    } else if(value < -1.0) {//マイナスの値を許容できるよう変更
        return -1.0;
    }
    return value;
 }
